...
solnp<-function(pars, Jfun, Efun=NULL, EQ=NULL, Ifun=NULL, ILB=NULL, IUB=NULL, LB=NULL,
		UB=NULL, control=list(), ...)
{
	# no.parameters= np
	tic=Sys.time()
	np=length(pars)
	...
	if(tc>0){
		...
		if(nineq>0){
			testmin=apply(cbind(constraint[(neq+1):tc]-ILB,IUB-constraint[(neq+1):tc]),1,FUN=function(x) min(x[1],x[2]))
	...
	while(iteration<maxit){
		...
		if(tc>0.5){
			...
			if(nineq>0.5){
				if(min(rbind(constraint[(neq+1):tc]-pb[1:nineq,1],
								pb[1:nineq,2]-constraint[(neq+1):tc])) > 0){

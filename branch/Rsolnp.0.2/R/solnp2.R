...
solnp2 <- function(pars, Jfun, Efun=NULL, EQ=NULL, Ifun=NULL, ILB=NULL, IUB=NULL, LB=NULL, UB=NULL, control=list(), ...)
{
	# start timer
	tic = Sys.time()
	...
	ofn <- quote(Jfun(.par,...))
	if(!is.null(Efun)) efn <- quote(Efun(.par,...) - EQ) else efn=NULL
	...
	while( iteration < maxit ){
		...
		if( lpb[ 2 ] == 0 ) {
			...
			# [ 1 neq np]
			xscale = c(xscale, rep( 1, length = length(p) ) )
		}
		
		xscale = apply( matrix(xscale, ncol = 1), 1, FUN = function( x ) min( max( abs(x), tol ), 1/tol ) )
		
		#assign(".par", p[(nineq+1):(nineq+np)], envir = .solenv)
		
		ans=try(.Call("solnp", pars=as.double(p), Ofn=ofn, Efn=efn, Ifn=ifn, xeq=EQ, xpb=pb, xscale=xscale, 
				control=ctrlv, rho=.solenv, xlagrange1=as.numeric(l), xlagrange2=as.double(mu), xhess=hess, xn=as.double(xn),
				obin=ob), silent=TRUE)

		if(inherits(ans,"try-error")){
			cat("\nThe solver terminated abnormally...exiting\n")
			return()
		}
	...
	# end timer
	toc = Sys.time() - tic
	ans = list(par = p, convergence = convergence, values = jh, lagrange = l, hessian = hess, xine = xineq0, elapsed = toc)
	return( ans ) 
}